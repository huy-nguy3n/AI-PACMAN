    ...
    def successorStates(self, state):
        ...
        **********************************
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            corners_notVisited = state[1]
            hitsWall = self.walls[nextx][nexty]

            # check if next state is a corner
            if (nextx, nexty) in corners_notVisited:
                corners_notVisited = list(filter(lambda x: x != (nextx, nexty), corners_notVisited))
                
            # not a wall
            if (not hitsWall):
                # Construct the successor.
                nextState = ((nextx, nexty), corners_notVisited)
...
def cornersHeuristic(state, problem):
    ...
    for corner in unvisitedCorners:
        heuristicvalue.append(manhattan(xy, corner))
...
def foodHeuristic(state, problem):
    ...
    for food in foodposition:
        # heuristic.append(manhattan(position,food))
        heuristic.append(maze(position, food, problem.startingGameState))
